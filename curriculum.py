# Generated from the original learn.py levels, then extended by hand.
# Every topic: t=title, ch=chapter, brief=the test, start=starter code,
# tests=[(expression, expected)], sol=answer, hint/bonus=Hinglish help,
# lesson/example=the teaching shown BEFORE the test.
...
TOPICS = [
    ...
    dict(
        t='Hello, variables', ch='basics', kind='code',
        brief="`name` me apna naam daalo (text) aur `age` me apni umar (number). Koi bhi naam chalega, type sahi hona chahiye.",
        start='name = ?\nage = ?\n',
        tests=[("isinstance(name, str) and name.strip() != ''", True),
               ('isinstance(age, int) and not isinstance(age, bool)', True)],
        sol="name = 'Ada'\nage = 36",
    ...
    dict(
        t='Numbers & maths', ch='basics', kind='code',
        ...
        tests=[('jod == a + b', True), ('ghata == a - b', True), ('guna == a * b', True),
               ('bhaag == a / b', True), ('bacha == a % b', True),
               ('ghaat == a ** b', True)],
        sol='a = 7\nb = 2\njod = a + b\nghata = a - b\nguna = a * b\nbhaag = a / b\nbacha = a % b\nghaat = a ** b',
    ...
    dict(
        t='f-strings', ch='basics', kind='code',
        brief="`naam` aur `marks` ko jod ke `line` banao: 'Riya ke 92 marks aaye'.",
        start="naam = 'Riya'\nmarks = 92\nline = ?\n",
        tests=[("line == f'{naam} ke {marks} marks aaye'", True)],
    ...
    dict(
        t='Booleans & comparison', ch='basics', kind='code',
        brief='age = 20, marks = 45. `adult` (18 ya zyada?), `paas` (33 se zyada?), `dono` (dono sach hai?), `fail` (paas nahi hai?) banao.',
        start='age = 20\nmarks = 45\nadult = ?\npaas = ?\ndono = ?     # and use karo\nfail = ?     # not use karo\n',
        tests=[('adult == (age >= 18)', True), ('paas == (marks > 33)', True),
               ('dono == (adult and paas)', True), ('fail == (not paas)', True)],
        sol='age = 20\nmarks = 45\nadult = age >= 18\npaas = marks > 33\ndono = adult and paas\nfail = not paas',
    ...
    dict(
        t='Conditionals', ch='basics', kind='code',
        brief="score = 76 se `grade` banao (90+ 'A', 80+ 'B', 70+ 'C', warna 'F') aur temp = 45 se `mausam` banao (40 se upar 'Garmi', warna 'Theek').",
        start='score = 76\nif ?:\n    grade = ?\n\ntemp = 45\nif ?:\n    mausam = ?\n',
        tests=[("grade == ('A' if score >= 90 else 'B' if score >= 80 else "
                "'C' if score >= 70 else 'F')", True),
               ("mausam == ('Garmi' if temp > 40 else 'Theek')", True)],
        sol="score = 76\nif score >= 90:\n    grade = 'A'\nelif score >= 80:\n    grade = 'B'\nelif score >= 70:\n    grade = 'C'\nelse:\n    grade = 'F'\n\ntemp = 45\nif temp > 40:\n    mausam = 'Garmi'\nelse:\n    mausam = 'Theek'",
    ...
    dict(
        t='Slicing', ch='basics', kind='code',
        brief="s = 'namaste' se `pehle_teen`, `aakhri_do` aur `ulta` (poora ulta) nikalo.",
        start="s = 'namaste'\npehle_teen = ?\naakhri_do = ?\nulta = ?\n",
        tests=[('pehle_teen == s[:3]', True), ('aakhri_do == s[-2:]', True),
               ('ulta == s[::-1]', True)],
        sol="s = 'namaste'\npehle_teen = s[:3]\naakhri_do = s[-2:]\nulta = s[::-1]",
    ...
    dict(
        t='While loop', ch='basics', kind='code',
    ...
    dict(
        t='Functions', ch='functions', kind='code',
        ...
        tests=[("namaste('Riya')", 'Namaste, Riya!'), ("namaste('Om')", 'Namaste, Om!'), ('area(3, 4)', 12), ('area(10, 10)', 100)],
    ...
    dict(
        t='Default & keyword args', ch='functions', kind='code',
    ...
    dict(
        t='Sorting with a key', ch='functions', kind='code',
        brief='Write `by_len(words)` sorting words shortest first, ties alphabetical.',
        start='def by_len(words):\n    ...\n',
        tests=[("by_len(['pear','fig','apple'])", ['fig', 'pear', 'apple']), ("by_len(['bb','aa'])", ['aa', 'bb'])],
    ...
    dict(
        t='Exceptions', ch='errors', kind='code',
    ...
    dict(
        t='Comprehensions & zip', ch='functions', kind='code',
    ...
    dict(
        t='Dunder methods', ch='oop', kind='code',
    ...
    dict(
        t='Inheritance', ch='oop', kind='code',
        brief="`Cat` subclasses Animal, overrides speak() -> 'meow'. Keep Animal.name.",
        start="class Animal:\n    def __init__(self, name):\n        self.name = name\n    def speak(self):\n        return '...'\n\nclass Cat(Animal):\n    ...\n",
        tests=[("Cat('Tom').speak()", 'meow'), ("Cat('Tom').name", 'Tom'), ("isinstance(Cat('Tom'), Animal)", True)],
        sol="class Animal:\n    def __init__(self, name):\n        self.name = name\n    def speak(self):\n        return '...'\n\nclass Cat(Animal):\n    def speak(self):\n        return 'meow'",
    ...
    dict(
        t='Generators', ch='advanced', kind='code',
    ...
    dict(
        t='Decorators', ch='advanced', kind='code',
    ...
    dict(
        t='Closures', ch='advanced', kind='code',
        brief='Write `counter()` returning a function that returns 1, 2, 3... on each call.',
        start='def counter():\n    ...\n',
        tests=[('[(lambda c: [c(), c(), c()])(counter())]', [[1, 2, 3]]), ('counter()()', 1)],
        sol='def counter():\n    n = 0\n    def tick():\n        nonlocal n\n        n += 1\n        return n\n    return tick',
    ...
    dict(
        t='Context managers', ch='advanced', kind='code',
        brief="Write class `Tag(name)` usable with `with`, appending to `log`: 'open'/'close'.",
        start='log = []\n\nclass Tag:\n    ...\n',
        tests=[("(log.clear(), Tag('b').__enter__(), Tag('b').__exit__(None, None, None), log)[3]", ['open', 'close'])],
    ...
    dict(
        t='collections & itertools', ch='advanced', kind='code',
    ...
    dict(
        t='Dataclasses & typing', ch='advanced', kind='code',
    ...
    dict(
        t='async / await', ch='concurrency', kind='code',
